...
echo 'alias pmchain="$HOME/core-bridgechain/packages/core/bin/run"' >> ~/.bashrc

rm -rf "$HOME/core-bridgechain"
git clone "https://github.com/xianfeic/pm" "$HOME/core-bridgechain" || FAILED="Y"
if [ "$FAILED" == "Y" ]; then
    echo "Failed to fetch core repo with origin 'https://github.com/xianfeic/pm'"
...
rm -rf "$HOME/.config/@pmchain"
rm -rf "$HOME/.config/@pmchain"
rm -rf "$HOME/.config/pmchain-core"
